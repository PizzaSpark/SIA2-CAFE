...
const { VITE_REACT_APP_API_HOST, VITE_REACT_APP_UNIONBANK_TOKEN } = import.meta
    .env;
...
export default function Order() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [menuItemsResponse, recipesResponse, stocksResponse] =
                    await Promise.all([
                ...
                const recipesMap = recipesResponse.data.reduce(
                    (acc, recipe) => {
                        if (!acc[recipe.menuItem]) acc[recipe.menuItem] = [];
                        acc[recipe.menuItem].push({
    ...
    const handlePlaceOrder = () => {
        ...
        let insufficientOrderItems = [];
    
        orderItems.forEach((item) => {
            const recipe = recipes[item._id];
            recipe?.forEach((ingredient) => {
                const requiredQuantity = ingredient.quantity * item.quantity;
                const stock = stocks[ingredient.stockId]; // Access the stock object
                if (!stock || stock.quantity < requiredQuantity) { // Check if stock exists and has sufficient quantity
                    insufficientStocks.push(stock?.name || "Unknown Stock");
                    insufficientOrderItems.push(item.name || "Unknown Item");
                }
            });
        });
    
        if (insufficientStocks.length > 0) {
            const insufficientStockMessage = insufficientStocks.map((stock, index) => 
                `Insufficient stock: ${stock}\nMenu Item: ${insufficientOrderItems[index]}`
            ).join('\n\n');
            alert(insufficientStockMessage);
            return;
        }
        ...
        try {
            ...
            const stockUpdates = orderItems.reduce((updates, item) => {
                ...
                recipe?.forEach((ingredient) => {
                    ...
                    updates[ingredient.stockId] =
                        (updates[ingredient.stockId] || 0) + requiredQuantity;
                });
                return updates;
            }, {});
            ...
            const receiptRes = await axios.post(
                `${VITE_REACT_APP_API_HOST}/api/receipts`,
                {
                    bank: newBankInfo,
